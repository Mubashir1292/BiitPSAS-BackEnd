...
namespace OfficialPSAS.Controllers
{
    ...
    public class PSASController : ApiController
    {
        ...
        OfficialSASEntities27 db = new OfficialSASEntities27();
        ...
        [HttpGet]
        public HttpResponseMessage FetchingTheNameAridNumber()
        {
            try
            {
                ...
                return Request.CreateResponse("id Did'nt Found "+id);
            }
            catch (Exception cp)
            {
                return Request.CreateResponse(cp.Message + ":" + cp.InnerException);
    ...
    public HttpResponseMessage PersonalDataInserting()
        ...
        public HttpResponseMessage ChekingGroupExistence(string id)
        {
            try
            {
                ...
                return Request.CreateResponse(alreadyGroupOrMember);
            }catch(Exception cp)
            {
                return Request.CreateResponse(cp.Message + ":" + cp.InnerException);
            }
            ...
            try
            {
                ...
                if (findingUser != null)
                {
                    ...
                    if (studentChecking != null)
                    {
                        ...
                        {
                            statusOfSingleStudent = "This Student is Available for group:"+studentChecking.users.username;
                        }
                        else
                        {
                            statusOfSingleStudent = "Student is not Available:"+studentChecking.users.username;
                        }
                    }
                    else
                    {
        ...
        public HttpResponseMessage GettingTechnolgiesOtherThenCreatorTechnology(string regNo)
        {
            try
            {
                ...
                if (IsUser != null)
                {
                    var isStudentTechnology = db.Student.Where(s => s.st_id == regNo).Select(s => s.GroupMember.Technology.name).FirstOrDefault();
                    if (isStudentTechnology != null)
                    {
                        var allTechnologiesOfSameGroupMembers = db.GroupMember.Select(s => s.Technology.name).ToList();
                        if (allTechnologiesOfSameGroupMembers.Count > 0)
                        {
                            var otherTechnologies = db.Technology
                            .Where(t => !allTechnologiesOfSameGroupMembers.Contains(t.name))
                            .ToList();
                                if (otherTechnologies.Any())
                                {
                                    return Request.CreateResponse(otherTechnologies.Select(s=>s.name));
                                }
                                else
                                {
                                    return Request.CreateResponse("");
...
*/       
        ...
        [HttpPost]
        public HttpResponseMessage SendingRequestToMember()
        {
            try
            {
                var data = HttpContext.Current.Request;
                ...
                string technology = data["technology"];
                ...
                if (receiverUser != null && senderUser != null)
                {
                    ...
                    if (senderGroup != null)
                    {
                        ...
                        {
                            var technologiesListOfSameGroupMembers = db.GroupMember.Where(s => s.group.gid == groupfindingOfSender).Select(s => s.Technology.name).ToList();
                            if (technologiesListOfSameGroupMembers.Count > 0)
                            {
                                var technologyFound = db.Technology.Where((s) => s.name == technology).FirstOrDefault();
                                if (!technologiesListOfSameGroupMembers.Contains(technologyFound.name))
                                {
